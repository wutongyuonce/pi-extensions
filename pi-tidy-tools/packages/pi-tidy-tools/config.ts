...
export function loadTidyState(
  options: { envValue?: string; configPath?: string } = {}
): TidyState {
  const envValue = options.envValue ?? process.env.PI_TIDY_TOOLS;
  const envEnabled = parseEnabled(envValue);
  if (envEnabled !== undefined)
    return { enabled: envEnabled, source: "environment" };

  const configPath = options.configPath ?? defaultConfigPath();
  try {
    const parsed = JSON.parse(readFileSync(configPath, "utf8"));
    if (typeof parsed?.enabled === "boolean")
      return { enabled: parsed.enabled, source: "file" };
  } catch {
...
export function loadTidyIcons(configPath = defaultConfigPath()): boolean {
  try {
    const parsed = JSON.parse(readFileSync(configPath, "utf8"));
    if (
      parsed &&
...
export function loadTidyMode(configPath = defaultConfigPath()): TidyMode {
  try {
...
export async function saveTidyEnabled(
  enabled: boolean,
...
export async function saveTidyMode(
  mode: TidyMode,
  configPath = defaultConfigPath()
): Promise<void> {
...
export async function saveTidyIcons(
  icons: boolean,
  configPath = defaultConfigPath()
): Promise<void> {
