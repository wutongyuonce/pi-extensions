...
const TOOL_BUDGET_BLOCKED_MESSAGE = /^Tool budget hard limit reached after (\d+) tool calls? \(hard (\d+)\)\. The '([^']+)' tool is blocked so you can finalize from the context you already have\.$/;

/**
 * Recognizes a hard-block reason generated by toolBudgetBlockedMessage for this exact budget
 * and tool. A tool result only counts as a budget block when the entire message matches the
 * runtime format and the embedded hard limit and tool name belong to this run; incidental
 * occurrences of the phrase inside ordinary tool output are rejected.
 */
export function isToolBudgetBlockedMessage(
	budget: ResolvedToolBudget,
	resultText: string,
	blockedTool: string | undefined,
): boolean {
	const tool = blockedTool?.trim();
	if (!tool) return false;
	const match = TOOL_BUDGET_BLOCKED_MESSAGE.exec(resultText.trim());
	if (!match) return false;
	const [, blockedToolCount, hard, namedTool] = match;
	return Number(hard) === budget.hard && Number(blockedToolCount) > budget.hard && namedTool === tool;
}
