...
test("palette picker previews the highlighted preset and restores on cancel", async () => {
  ...
  try {
    writeFileSync(join(root, "pi-statusline.json"), JSON.stringify({ palettePreset: "sunset", segments: ["model"] }));
    const mock = createMockPi();
    statusline(mock.pi);
...
test("a replacement session reloads JSON settings and uses configured segment text", async () => {
  ...
  try {
    ...
    writeFileSync(path, JSON.stringify({ segments: ["model"], segmentText: { model: { prefix: "Model: " } } }));
    const mock = createMockPi();
    statusline(mock.pi);
    const context = createMockContext({
      mode: "tui",
    ...
    writeFileSync(path, JSON.stringify({ segments: ["cwd"], segmentText: { cwd: { suffix: "!" } } }));
    await emit(mock.events, "session_start", {}, context.ctx);
    footer = createFooter(context.footer as FooterFactory);
    assert.match(footer.render(200).join("\n"), /📁 \/workspace\/project!/u);
    assert.doesNotMatch(footer.render(200).join("\n"), /sonnet-4/u);
