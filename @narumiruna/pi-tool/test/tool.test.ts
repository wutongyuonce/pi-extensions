...
test("a delayed settings load cannot overwrite a replacement session", async () => {
  ...
  toolExtension(mock.pi, {
    ...
    readSettings: async () => {
      reads += 1;
      return reads === 1 ? firstLoad : { kind: "loaded" as const, settings: { activeToolStatus: true } };
    },
  });
...
test("terminal controls are stripped by the browse display boundary", async () => {
  ...
  rpc.assertConsumed();
  assert.equal(
    rpc.dialogs.every((dialog) => !dialog.title.includes("\u001b") && !dialog.title.includes("\u0007")),
    true,
  );
});
...
test("duplicate sanitized labels keep stable raw tool identity in RPC", async () => {
  ...
  const secondPath = catalog.items[1]?.detailContent.split("\n").find((line) => line.startsWith("Path: "));
