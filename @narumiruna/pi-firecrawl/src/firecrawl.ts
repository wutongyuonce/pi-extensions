...
async function handleFirecrawlCommand(pi: ExtensionAPI, args: string, ctx: CommandContext, generation: number) {
  ...
  ctx.ui.notify(
    sanitizeFirecrawlDisplay(`Unknown /firecrawl command: ${args.trim()}\n\n${buildCommandGuide()}`),
    "warning",
  );
}
