...
test("firecrawl display sanitization strips terminal controls and remains bounded", () => {
  assert.equal(sanitizeFirecrawlDisplay("safe\u001b]8;;https://evil\u0007link\u001b]8;;\u0007"), "safelink");
...
test("firecrawl unknown-command feedback sanitizes terminal controls in UI modes", async () => {
  ...
  await assert.rejects(
    ...
    (error: Error) =>
      error.message.includes("badroute") && !error.message.includes("\u001b") && !error.message.includes("\u0007"),
  );
});
...
test("jsonResult gives concurrent truncated responses unique artifact paths", async () => {
  ...
  const [first, second] = await Promise.all([jsonResult(payload, artifactOwner), jsonResult(payload, artifactOwner)]);
...
test("response artifact cleanup is isolated by session owner", async () => {
  ...
  const secondOwner = {};
  const payload = { value: "x".repeat(DEFAULT_MAX_BYTES + 1) };
  const [first, second] = await Promise.all([jsonResult(payload, firstOwner), jsonResult(payload, secondOwner)]);
  assert.ok(first.details.fullResponsePath);
  assert.ok(second.details.fullResponsePath);
...
test("all five Firecrawl tools bound oversized successful responses", async () => {
  ...
  try {
    ...
    assert.deepEqual(paths, ["/v1/scrape", "/v1/crawl", "/v1/crawl/crawl-id", "/v1/map", "/v1/search"]);
  } finally {
...
test("firecrawl bounds oversized non-2xx responses and saves their exact raw body", async () => {
  ...
  try {
    ...
    await assert.rejects(firecrawlRequest("GET", hugePath, undefined, undefined, artifactOwner), (error: Error) => {
...
test("session shutdown rejects an error artifact write that starts after cleanup", async () => {
  ...
  const pendingRequest = firecrawlRequest("GET", "/late-error", undefined, undefined, artifactOwner);
  try {
    await cleanupResponseArtifacts(artifactOwner);
  ...
  await withTempAgentDir(async (agentDir) => {
    ...
    const { ctx } = createMockContext({
      ...
      custom: async (factory: unknown) => {
        ...
        toggledRowKeptCursor = Boolean(renders[1]?.some((line) => line.includes("› [ ] firecrawl_crawl")));
        return result;
      },
    });
...
test("catalog changes unload unavailable tools and leave newly available tools deferred", async () => {
  await withTempAgentDir(async () => {
    ...
    await loader.execute("loader-search", { query: "search the web" }, new AbortController().signal, undefined, ctx);
    ...
    assert.deepEqual(mock.rawPi.getActiveTools(), ["other_tool", LOAD_TOOL]);
    assert.deepEqual(readSettings(process.env.PI_CODING_AGENT_DIR ?? "", NEW_SETTINGS_FILE).tools, CAPABILITY_TOOLS);
  });
});
