...
function record(event) {
  if (process.env.PI_LSP_TEST_LOG) {
    appendFileSync(process.env.PI_LSP_TEST_LOG, `${JSON.stringify({ pid: process.pid, ...event })}\n`);
  }
}
if (lifecycle) {
...
function handle(message) {
  ...
  if (message.method === "textDocument/didOpen") {
    ...
    if (scenario === "push-alternate-uri-encoding") {
      setTimeout(() => publish(alternateEncoding(uri), [diagnostic("alternate uri encoding diagnostic")]), 40);
