...
async function ensureSafeScreenshotParent(resolvedPath: ResolvedScreenshotPath) {
  ...
  if (!rootPath) {
    throw new Error("Screenshot savePath parent must stay inside the current working directory or OS temp directory.");
  }
...
export function selectAllowedRoot(path: string, roots: readonly string[]) {
  ...
  matchingRoots.sort(
    (left, right) =>
      normalizePathForComparison(resolve(right)).length - normalizePathForComparison(resolve(left)).length,
  );
...
async function assertSafeScreenshotTargetPath(resolvedPath: ResolvedScreenshotPath) {
  ...
  if (!realAllowedRoots.some((root) => isPathInsideRoot(realTargetPath, root))) {
    throw new Error("Screenshot savePath resolves outside the current working directory or OS temp directory.");
  }
}
...
async function assertPathWithinRealRoot(path: string, realRootPath: string) {
  ...
  if (!isPathInsideRoot(realPath, realRootPath)) {
    throw new Error("Screenshot savePath parent resolves outside the current working directory or OS temp directory.");
  }
}
...
function shouldRetryRenameAfterRemovingDestination(error: unknown) {
  return process.platform === "win32" && isNodeError(error) && ["EACCES", "EEXIST", "EPERM"].includes(error.code ?? "");
}
