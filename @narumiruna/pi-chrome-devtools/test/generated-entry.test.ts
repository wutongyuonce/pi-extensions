...
test("declared generated entry preserves registration and partial lifecycle cleanup", async () => {
  ...
  try {
    ...
    const webMcpListTool = mock.tools.find((tool) => tool.name === "chrome_devtools_webmcp_list_tools") as
      | { execute: (...args: unknown[]) => Promise<unknown> }
      | undefined;
    assert.ok(webMcpListTool);
    await assert.rejects(
      webMcpListTool.execute("generated-webmcp", {}, new AbortController().signal, undefined, context.ctx),
