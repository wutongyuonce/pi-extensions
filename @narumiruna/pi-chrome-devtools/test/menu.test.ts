...
import { applyAvailableChromeDevtoolsTools, setChromeDevtoolsSessionOwner } from "../src/lazy-tools.js";
...
test("browser status distinguishes unobserved, running, exited, and failed states without probing", () => {
  ...
  applyRuntimeWebMcpSetting(true, owner);
  assert.match(buildBrowserStatusMessage(owner), /attached browser profiles.*authenticated sessions/u);
  applyRuntimeWebMcpSetting(false, owner);
