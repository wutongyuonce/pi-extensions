...
test.each(cases)(
  ...
  async (kind) => {
    ...
    const answer = Array.from({ length: 48 }, (_, index) => `needle ${kind} transcript row ${index + 1}`).join("\n");
    ...
    const running = runBtwFullscreen(harness.ctx, (fullscreenCtx) =>
      fullscreenCtx.ui.custom<"closed">((tui, theme, _keys, done) => {
        ...
        return new BtwAnsweringView(tui, theme, turns, "current question", () => done("closed"), undefined, {
