# Contributing to @tintinweb/pi-tasks

This guide exists to save both sides time.

## Philosophy

`pi-tasks` is a [pi](https://pi.dev) extension, and it tries to stay focused:
track multi-step work and coordinate it across sessions in a way that feels
native to pi, and do that well. Features that don't serve that goal, or that
bolt on unrelated complexity, are likely to be declined. When in doubt, open an
issue and discuss the idea before writing the code.
...
- For bugs, include a minimal repro: pi version, this extension's version, your
  task config, the steps, and the actual vs. expected behavior.
...
All four must pass. `npm run lint:fix` will auto-fix most style issues.
...
Open an [issue](https://github.com/tintinweb/pi-tasks/issues) — questions and
