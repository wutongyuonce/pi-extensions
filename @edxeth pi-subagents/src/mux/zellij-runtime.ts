...
export function resolveZellijTargetFromSessions(
	...
	const preferred = piCandidates.find(({ name }) => name === preferredSessionName)
...
const PANE_ACTIONS = new Set([
	"close-pane", "dump-screen", "move-pane", "rename-pane", "write", "write-chars",
]);
...
export async function closeZellijSurface(
	surface: string,
	target?: ZellijTarget,
): Promise<void> {
...
export async function isZellijSurfaceLive(
	target: ZellijTarget,
	surface: string,
): Promise<boolean> {
	const panes = JSON.parse(await runZellijAction(target, ["list-panes", "--all", "--json"]));
	return Array.isArray(panes) && panes.some(
