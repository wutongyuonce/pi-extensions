...
export const GUARD_COMMANDS = [
  ["status", "Show full Guard runtime status"], ["dcg", "Use pure DCG mode for this Pi run"],
  ["sandbox", "Use DCG + OS sandbox for this Pi run"], ["init", "Create the default project config"],
];
...
export function renderStatus(status) {
  const lines = [
    ...
    `Bash policy: ${status.bashPolicy === "dcg-error" ? "DCG (error, retrying)" : status.bashPolicy === "dcg-missing" ? "DCG (missing — install dcg)" : status.bashPolicy === "dcg" ? "DCG" : "built-in"}`,
  ...
  async function handleCommand(args, ctx) {
    ...
    try {
      ...
      if (command === "i" || command === "init") {
        ...
        await guard.setDcgEnabled(true); await guard.setSandboxEnabled(true); ctx.ui.notify("Sandbox mode for this Pi run: DCG judges, OS sandbox enforces.", "success");
        ...
        await guard.setDcgEnabled(true); await guard.setSandboxEnabled(false); ctx.ui.notify("DCG mode for this Pi run: Bash judged by DCG only.", "success");
        ...
        ctx.ui.notify("Usage: /guard status|init|dcg|sandbox", "warning");
      }
      updateStatus(ctx);
