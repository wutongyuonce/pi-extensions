...
export const DCG_MISSING_REASON = "DCG is not installed. Install it with `brew install dicklesworthstone/tap/dcg` (no `dcg install` hook setup needed for Pi), then reload. Bash stays blocked until DCG is available.";

export function formatGuardFooter(status) {
  if (!status.guardActive) return "[Guard: OFF]";
  const policy = status.dcgError ? "DCG:error" : status.bashPolicy === "dcg-missing" ? "DCG:missing" : status.usingDcg ? "DCG" : "built-in";
...
export function createGuardController({ cwd, sandbox, dcg = createDcgClient() } = {}) {
  ...
  function getStatus() {
    ...
    const status = {
      ...
      bashPolicy: active.dcg && state.dcgAvailability === "missing" ? "dcg-missing" : active.dcg && state.dcgError ? "dcg-error" : active.dcg && state.dcgAvailability === "available" ? "dcg" : "built-in",
  ...
  async function handleToolCall({ toolName, input, hasUI, requestApproval, notify }) {
    ...
    if (toolName === "bash" && active.dcg && state.dcgAvailability === "missing") {
      const detection = await detectDcgAvailability();
      if (detection.kind === "error") {
        const firstError = !state.dcgErrorNotified;
        const decision = await applyDcgAction("error", { reason: detection.reason }, { hasUI, requestApproval, notify: firstError ? notify : undefined });
      ...
      if (detection.kind === "missing") {
        return { status: "block", block: true, reason: DCG_MISSING_REASON };
      }
    }
