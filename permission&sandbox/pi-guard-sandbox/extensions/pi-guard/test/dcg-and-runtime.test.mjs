...
test("missing DCG blocks bash with an install hint instead of built-in policy", async () => {
  const cwd = workspace();
  ...
  const guard = createGuardController({ cwd, sandbox: sandbox(), dcg });
  await guard.refresh();
  assert.equal(guard.getStatus().bashPolicy, "dcg-missing");
  assert.match(guard.getStatus().footer, /DCG:missing/);
  for (const command of ["sudo echo nope", "ls -la"]) {
    const result = await guard.handleToolCall({ ...input, input: { command } });
