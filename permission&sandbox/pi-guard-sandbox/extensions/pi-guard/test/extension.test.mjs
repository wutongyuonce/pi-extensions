...
test("TUI registers guarded tool, status UI, slash status, and autocomplete", async () => {
  ...
  assert.deepEqual(getGuardArgumentCompletions("s").map((item) => item.value), ["status", "sandbox"]);
  assert.deepEqual(getGuardArgumentCompletions("").map((item) => item.value), ["status", "dcg", "sandbox", "init"]);
  assert.equal(getGuardArgumentCompletions("sandbox o"), null);
});

test("two-word mode commands switch between dcg and sandbox", async () => {
  const cwd = mkdtempSync(join(tmpdir(), "pi-guard-mode-"));
  await writeConfig(cwd, createDefaultConfig());
  const pi = fakePi();
  registerPiGuard(pi, dependencies);
  const ctx = context("tui", cwd);
  await pi.events.get("session_start")({}, ctx);
  await pi.commands[0].handler("dcg", ctx);
  await pi.commands[0].handler("status", ctx);
  assert.match(ctx.notices.at(-1)[0], /Sandbox: off/);
  await pi.commands[0].handler("sandbox", ctx);
  await pi.commands[0].handler("status", ctx);
  assert.match(ctx.notices.at(-1)[0], /Sandbox: enabled/);
});
