...
DCG is required for Bash protection. Install only the binary — Pi needs no `dcg install` hook setup:

```bash
brew install dicklesworthstone/tap/dcg
```

Without Homebrew (binary only, no hook configuration):

```bash
curl -fsSL "https://raw.githubusercontent.com/Dicklesworthstone/destructive_command_guard/main/install.sh" | bash -s -- --no-configure
```

Verify with `dcg --version`. Without DCG on `PATH`, Guard blocks Bash with an install hint.
...
```
/guard init              → initialize — creates config, DCG mode on
/guard status            → check DCG and Guard state
/guard sandbox           → use DCG + OS sandbox for this run
/guard dcg               → back to pure DCG mode for this run
```
...
| `/guard dcg` | Pure DCG mode for this Pi run (default) |
| `/guard sandbox` | DCG + OS sandbox for this Pi run |

Older switches (`on`/`off`, `sandbox on`/`off`, `dcg on`/`off`, `read-only`/`workspace-write`, `network on`/`off`) still work as runtime overrides.

All slash-command changes are runtime overrides: they never rewrite `.pi/pi-guard.json`, and a new Pi run restores project defaults. `/guard <Tab>` offers the canonical commands. `/guard status` lists the active overrides; the compact footer shows states such as `[Guard: workspace-write · DCG]`, `[Guard: workspace-write · DCG:missing]`, `[Guard: sandbox-off · built-in]`, and `[Guard: OFF]`. `/guard dcg` and `/guard sandbox` switch the Bash chain for this run.
...
| `Guard: ... · DCG:missing` | DCG binary not found | `brew install dicklesworthstone/tap/dcg`, then `/reload` |
